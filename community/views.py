...
def discussion_list(request, dataset_id):
    ...
    """
    discussions = Discussion.objects.filter(dataset_id=dataset_id).order_by('-created_at')
    paginator = Paginator(discussions, 10)  # Show 10 discussions per page
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)

    return render(request, 'community/discussion_list.html', {'page_obj': page_obj})
...
def discussion_detail(request, slug):
    ...
    if request.method == 'POST':
        ...
        if comment_form.is_valid():
            ...
            comment.save()
            return redirect(reverse('discussion_detail', args=[discussion.slug]))
    else:
...
def edit_comment(request, comment_id):
    ...
    if not comment.is_editable():
        return HttpResponseForbidden("The edit window for this comment has expired.")

    if request.method == 'POST':
        form = CommentForm(request.POST, instance=comment)
        if form.is_valid():
            form.save()
            return redirect(reverse('discussion_detail', args=[comment.discussion.slug]))
    else:
        form = CommentForm(instance=comment)

    return render(request, 'discussions/edit_comment.html', {'form': form, 'comment': comment})
